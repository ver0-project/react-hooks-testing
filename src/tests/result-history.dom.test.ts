...
describe('result history DOM', () => {
	...
	test('should capture all renders states of hook', async () => {
		...
		expect(result.all).toEqual([{value: 0}, {value: 1}, {error: Error('expected')}])

		await rerender(3)

		expect(result.value).toBe(3)
		expect(result.all).toEqual([{value: 0}, {value: 1}, {error: Error('expected')}, {value: 3}])

		await rerender()

		expect(result.value).toBe(3)
		expect(result.all).toEqual([{value: 0}, {value: 1}, {error: Error('expected')}, {value: 3}, {value: 3}])
	})
})
...
describe('result history SSR hydrated', () => {
	...
	test('should capture all renders states of hook with hydration', async () => {
		const {result, hydrate, rerender} = await renderHookServer(
		...
		// "fun part" is that react tries to render errored component twice, therefore we have two errors
		expect(result.error).toEqual(Error('expected'))
		expect(result.all).toEqual([{value: 0}, {value: 0}, {value: 1}, {error: Error('expected')}])

		await rerender(3)

		expect(result.value).toBe(3)
		expect(result.all).toEqual([{value: 0}, {value: 0}, {value: 1}, {error: Error('expected')}, {value: 3}])

		await rerender()

		expect(result.value).toBe(3)
		expect(result.all).toEqual([{value: 0}, {value: 0}, {value: 1}, {error: Error('expected')}, {value: 3}, {value: 3}])
	})
})
